  ...
  try {
    ...
    if (validateRequiredColumns) {
      ...
      if (importRecords.length === 0) {
        throwError(
          "There are no import records found. The required column validation could not be done"
        );
      }
      const csvColumns = Object.keys(importRecords[0]);
      const propertyMappingsRequiredKeys = propertyMappings
        .filter((propertyMap) => propertyMap.key.endsWith("*"))
        .map((propertyMap) => propertyMap.key.replace("*", ""));
      // Check if all required values are present
      const missingValues = propertyMappingsRequiredKeys.filter(
        (requiredValue) => !csvColumns.includes(requiredValue)
      );
